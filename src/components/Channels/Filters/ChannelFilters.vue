<template>
  <div :class="['filters', showFilterPort ? 'layer' : null]">
    <SortFilter />
    <div class="favs" tabindex="0">
      <img src="@/assets/icons/heart.svg" alt="heart icon" />
      <span>Избранные</span>
    </div>
    <div class="filters-wrap">
      <div class="selected-filter" v-if="selectedFilter">
        <p>{{ selectedFilter }}</p>
        <router-link to="/">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
            <path
              d="M12.0002 10.586L16.9502 5.636L18.3642 7.05L13.4142 12L18.3642 16.95L16.9502 18.364L12.0002 13.414L7.05023 18.364L5.63623 16.95L10.5862 12L5.63623 7.05L7.05023 5.636L12.0002 10.586Z"
            ></path>
          </svg>
        </router-link>
      </div>
      <svg
        v-else
        @click="showFilterPort = !showFilterPort"
        class="filter-svg"
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="rgb(235,235,235)"
      >
        <path
          d="M6.17 18C6.3766 17.4145 6.75974 16.9074 7.2666 16.5488C7.77346 16.1902 8.37909 15.9976 9 15.9976C9.62091 15.9976 10.2265 16.1902 10.7334 16.5488C11.2403 16.9074 11.6234 17.4145 11.83 18H22V20H11.83C11.6234 20.5855 11.2403 21.0926 10.7334 21.4512C10.2265 21.8099 9.62091 22.0025 9 22.0025C8.37909 22.0025 7.77346 21.8099 7.2666 21.4512C6.75974 21.0926 6.3766 20.5855 6.17 20H2V18H6.17ZM12.17 11C12.3766 10.4145 12.7597 9.90744 13.2666 9.5488C13.7735 9.19015 14.3791 8.99756 15 8.99756C15.6209 8.99756 16.2265 9.19015 16.7334 9.5488C17.2403 9.90744 17.6234 10.4145 17.83 11H22V13H17.83C17.6234 13.5855 17.2403 14.0926 16.7334 14.4512C16.2265 14.8099 15.6209 15.0025 15 15.0025C14.3791 15.0025 13.7735 14.8099 13.2666 14.4512C12.7597 14.0926 12.3766 13.5855 12.17 13H2V11H12.17ZM6.17 4.00001C6.3766 3.41448 6.75974 2.90744 7.2666 2.5488C7.77346 2.19015 8.37909 1.99756 9 1.99756C9.62091 1.99756 10.2265 2.19015 10.7334 2.5488C11.2403 2.90744 11.6234 3.41448 11.83 4.00001H22V6.00001H11.83C11.6234 6.58554 11.2403 7.09258 10.7334 7.45122C10.2265 7.80986 9.62091 8.00246 9 8.00246C8.37909 8.00246 7.77346 7.80986 7.2666 7.45122C6.75974 7.09258 6.3766 6.58554 6.17 6.00001H2V4.00001H6.17ZM9 6.00001C9.26522 6.00001 9.51957 5.89465 9.70711 5.70712C9.89464 5.51958 10 5.26523 10 5.00001C10 4.73479 9.89464 4.48044 9.70711 4.2929C9.51957 4.10537 9.26522 4.00001 9 4.00001C8.73478 4.00001 8.48043 4.10537 8.29289 4.2929C8.10536 4.48044 8 4.73479 8 5.00001C8 5.26523 8.10536 5.51958 8.29289 5.70712C8.48043 5.89465 8.73478 6.00001 9 6.00001ZM15 13C15.2652 13 15.5196 12.8947 15.7071 12.7071C15.8946 12.5196 16 12.2652 16 12C16 11.7348 15.8946 11.4804 15.7071 11.2929C15.5196 11.1054 15.2652 11 15 11C14.7348 11 14.4804 11.1054 14.2929 11.2929C14.1054 11.4804 14 11.7348 14 12C14 12.2652 14.1054 12.5196 14.2929 12.7071C14.4804 12.8947 14.7348 13 15 13ZM9 20C9.26522 20 9.51957 19.8947 9.70711 19.7071C9.89464 19.5196 10 19.2652 10 19C10 18.7348 9.89464 18.4804 9.70711 18.2929C9.51957 18.1054 9.26522 18 9 18C8.73478 18 8.48043 18.1054 8.29289 18.2929C8.10536 18.4804 8 18.7348 8 19C8 19.2652 8.10536 19.5196 8.29289 19.7071C8.48043 19.8947 8.73478 20 9 20Z"
        ></path>
      </svg>

      <OnClickOutside @trigger="showFilterPort = false">
        <transition name="filter">
          <div class="abs" v-if="showFilterPort || width > 920">
            <div class="abs-nav" v-if="showFilterPort">
              <p>Фильтры</p>

              <svg
                @click="showFilterPort = false"
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                fill="rgb(168,168,168)"
                viewBox="0 0 24 24"
              >
                <path
                  d="M12.0002 10.586L16.9502 5.636L18.3642 7.05L13.4142 12L18.3642 16.95L16.9502 18.364L12.0002 13.414L7.05023 18.364L5.63623 16.95L10.5862 12L5.63623 7.05L7.05023 5.636L12.0002 10.586Z"
                ></path>
              </svg>
            </div>
            <StyleFilter :genres="genres" :showFilterPort="showFilterPort" />
            <ThemeFilter :tags="tags" :showFilterPort="showFilterPort" />
          </div>
        </transition>
      </OnClickOutside>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, watch, computed } from 'vue'

import SortFilter from '@/components/Channels/Filters/SortFilter.vue'
import StyleFilter from '@/components/Channels/Filters/StyleFilter.vue'
import ThemeFilter from '@/components/Channels/Filters//ThemeFilter.vue'
import getStations from '@/composables/getStations'

import { OnClickOutside } from '@vueuse/components'
import { useWindowSize } from '@vueuse/core'
import { useRoute } from 'vue-router'

const { width } = useWindowSize()

const { tags, genres } = getStations()
const route = useRoute()

const showFilterPort = ref(false)

const body = document.querySelector('body')

const selectedFilter = computed(() => {
  if (route.params.style && width.value < 920) {
    return route.params.style.slice(14) as string
  } else if (route.params.theme && width.value < 920) {
    return route.params.theme.slice(14) as string
  } else return null
})

watch(showFilterPort, () => {
  if (showFilterPort.value == true) body?.classList.add('overflow')
  else body?.classList.remove('overflow')
})

watch(width, () => {
  if (width.value > 920) showFilterPort.value = false
})
</script>

<style lang="scss">
@import '@/globals';
.filters {
  display: flex;
  gap: 0.8rem;

  &.layer {
    z-index: 1007;
    &::before {
      content: '';
      top: 0;
      left: 0;
      width: 100%;
      height: 300vh;
      background-color: rgba($color-bg, 0.7);
      position: absolute;
      z-index: 1000;
    }
  }
  .favs,
  .selected-filter {
    display: flex;
    gap: 1rem;
    padding: 0.8rem 1.6rem;
    align-items: center;

    img {
    }

    span {
      font-weight: 600;
    }
  }

  .filters-wrap {
    display: flex;
    align-items: center;

    .selected-filter {
      display: flex;
      background-color: $color-main;
      p {
      }

      svg {
        width: 2rem;
        height: 2rem;
        fill: $color-main;
        padding: 3px;
        border-radius: 100%;
        background-color: rgba($color-white, 0.7);
      }
    }

    .filter-svg {
      display: none;

      @include respond(tab-port) {
        display: flex;
        align-items: center;
        border-radius: 0.8rem;
        background-color: $color-bg-2;
        padding: 0.8rem;
      }
    }
    .abs {
      display: flex;
      gap: 0.8rem;
      z-index: 1006;

      @include respond(tab-port) {
        padding: 1.6rem 1.6rem 4.8rem;
        flex-direction: column;
        box-sizing: border-box;
        position: fixed;
        background-color: $color-bg;
        bottom: 0;
        left: 0;
        z-index: 2000;
        width: 100%;
      }
    }

    .abs-nav {
      display: flex;
      justify-content: space-between;

      p {
        font-weight: 600;
        font-size: 1.8rem;
        margin-bottom: 1.6rem;
      }
    }
  }
}

.filter-wrap,
.favs,
.style-wrap,
.theme-wrap,
.selected-filter {
  background-color: $color-bg-2;
  transition: all 0.1s linear;
  border-radius: 0.8rem;
  cursor: pointer;
  z-index: 35;

  &:hover {
    background-color: $color-bg-3;
  }

  &:focus,
  &:active {
    outline: none;
    box-shadow: inset 0 0 0 2px rgba(#fff, 0.4);
  }
}
</style>
